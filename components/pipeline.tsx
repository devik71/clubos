...
export function Archive() {
  ...
  return <div className="archive"><div className="panel-heading"><span>GARBAGE GENIE / ARCHIVIST</span><DemoLabel /></div><Segmented label="Фази роботи з архівом" options={['01 Index', '02 Verify', '03 Reorganize']} value={phase} onChange={setPhase} /><div className="archive-body"><div className={`archive-files ${indexed ? 'indexed' : ''}`}><span className="micro">{indexed ? 'ІНДЕКС / ДЖЕРЕЛА НЕ ЗМІНЕНІ' : 'РОКИ МАТЕРІАЛІВ'}</span>{files.map((name, i) => <div key={name}><span className="file-icon">{i === 3 ? '↳' : '▤'}</span><span>{name}</span><small>{indexed ? i === 6 ? '? DUPLICATE' : 'INDEXED' : '—'}</small></div>)}</div><div className="archive-result"><span className="micro">{['СПОЧАТКУ ЗРОЗУМІТИ', 'ПОТІМ ПЕРЕВІРИТИ', 'ЛИШЕ ТОДІ ВПОРЯДКУВАТИ'][phase]}</span><h3>{['Порядок починається з індексу.', 'Сумнів — привід запитати.', 'План стає структурою.'][phase]}</h3><p>{['Папки, метадані, час, відео, аудіо, проєкти, рендери та історія подій. Система шукає зв’язки, зберігаючи оригінали.', 'Можливий дублікат не означає зайвий файл. Перевіряємо прив’язку до події та артиста. Невідоме зберігається окремо.', 'Після перевірки — контрольована реорганізація. Для нових записів Archivist підтримує порядок постійно.'][phase]}</p>{phase === 0 ? <button className="primary-button" onClick={() => setIndexed(true)}>{indexed ? '✓ Індекс побудовано' : 'Побудувати демо-індекс ↗'}</button> : phase === 1 ? <button className="primary-button" disabled={!indexed} onClick={() => setReviewed(true)}>{reviewed ? '✓ Зв’язки перевірено' : indexed ? 'Підтвердити зв’язки; зберегти невідоме' : 'Спочатку побудуйте індекс'}</button> : <button className="primary-button" disabled={!reviewed} onClick={() => setApplied(true)}>{applied ? '✓ Демо-план застосовано' : reviewed ? 'Застосувати демо-план ↗' : 'Спочатку перевірте зв’язки'}</button>}{indexed && <div className="archive-tree" role="status"><strong>BRUKXT / 2025-04-12</strong><span>├ Artist A</span><small>│　CAM_A · CAM_B · AUDIO_RAW<br />│　PROJECT · MASTER</small><span>├ Artist B</span><span>└ Needs review / copy (2).mp4</span><b>{applied ? 'Реорганізовано в симуляції. Невідоме збережено.' : reviewed ? 'Зв’язки підтверджено. Невідоме не видаляється.' : 'Запропонована структура · очікує перевірки'}</b></div>}</div></div><div className="archive-bottom">Жодних тихих видалень. Жодних випадкових перейменувань.<span>INDEX → VERIFY → REORGANIZE</span></div></div>;
}
